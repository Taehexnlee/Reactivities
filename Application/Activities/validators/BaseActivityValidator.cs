...
public class BaseActivityValidator<T, TDto> : AbstractValidator<T> where TDto : BaseActivityDto
{
    public BaseActivityValidator(Func<T, TDto> selector)
    {
        ...
        RuleFor(x => selector(x).Category).NotEmpty().WithMessage("Category is required").MaximumLength(50).WithMessage("Category cannot exceed 50 characters");
        RuleFor(x => selector(x).City).NotEmpty().WithMessage("City is required").MaximumLength(50).WithMessage("City cannot exceed 50 characters");
        RuleFor(x => selector(x).Venue).NotEmpty().WithMessage("Venue is required").MaximumLength(100).WithMessage("Venue cannot exceed 100 characters");
