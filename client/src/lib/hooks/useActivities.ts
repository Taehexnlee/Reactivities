...
export const useActivities = (id?: string) => {
  ...
  const {
    ...
    fetchNextPage,
    hasNextPage,
  } = useInfiniteQuery<PagedList<Activity, string>, Error, InfiniteData<PagedList<Activity, string>>, ["activities", string, string], ActivitiesCursor>({
    queryKey: ["activities", filter, startDate],
    queryFn: async ({ pageParam = null }: { pageParam?: ActivitiesCursor }) => {
    ...
    select: (data): InfiniteData<PagedList<Activity, string>> => ({
      ...data,
  ...
  const updateActivity = useMutation({
    mutationFn: async (activity: EditActivityRequest) => {
  ...
  const updateAttendance = useMutation({
    ...
    onMutate: async (activityId: string) => {
      ...
      const preActivity =
        queryClient.getQueryData<Activity>(["activities", activityId]);
